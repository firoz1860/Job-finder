...
// const RecruiterInfo = require("../db/RecruiterInfo");
...
router.get("/jobs", jwtAuth, async (req, res) => {
  try {
    ...
    if (req.query.q && req.query.q.trim()) {
      findParams.title = {
        $regex: new RegExp(req.query.q.trim(), "i"),
      };
    }
    ...
    // Sorting: Ascending
    if (req.query.asc) {
      const fields = Array.isArray(req.query.asc) ? req.query.asc : [req.query.asc];
      fields.forEach((key) => (sortParams[key] = 1));
    }

    // Sorting: Descending
    if (req.query.desc) {
      const fields = Array.isArray(req.query.desc) ? req.query.desc : [req.query.desc];
      fields.forEach((key) => (sortParams[key] = -1));
    }
...
router.get("/applicants", jwtAuth, (req, res) => {
  ...
  if (req.query.jobId) {
    console.log("Received jobId:", req.query.jobId);
    try {
      findParams.jobId = new mongoose.Types.ObjectId(req.query.jobId);
    } catch (err) {
      console.error("Invalid jobId format:", err.message);
  ...
  console.log("Final findParams:", findParams);
  console.log("Final sortParams:", sortParams);
...
router.put("/applicants", jwtAuth, async (req, res) => {
  try {
    ...
    res.json({ message: "Applicant updated", data: updatedApplicant });
  } catch (err) {
...
// //   if (req.query.jobType) {
// //     let jobTypes = Array.isArray(req.query.jobType)
// //       ? req.query.jobType
// //       : [req.query.jobType];
// //     findParams.jobType = { $in: jobTypes };
// //   }
...
// //   // if (req.query.salaryMin && req.query.salaryMax) {
// //   //   findParams.$and = [
// //   //     { salary: { $gte: parseInt(req.query.salaryMin) } },
// //   //     { salary: { $lte: parseInt(req.query.salaryMax) } },
// //   //   ];
...
// //   if (Object.keys(sortParams).length > 0) {
// //     aggregation.push({ $sort: sortParams });
// //   }

// //   console.log("📊 Aggregation Pipeline:", JSON.stringify(aggregation, null, 2));
...
//       {
//         $lookup: {
//           from: "recruiterinfos", // must match RecruiterInfo model
//           localField: "userId",
//           foreignField: "userId",
//           as: "recruiter",
//         },
...
//         $unwind: {
//           path: "$recruiter",
//           preserveNullAndEmptyArrays: true, // ensures jobs without recruiter info still show
//         },
//       },
//       { $match: findParams },
//     ];
...
//   // if (req.query.asc) {
//   //   const ascFields = Array.isArray(req.query.asc) ? req.query.asc : [req.query.asc];
//   //   ascFields.forEach((field) => {
//   //     sortParams[field] = 1;
//   //   });
//   // }

//   // if (req.query.desc) {
//   //   const descFields = Array.isArray(req.query.desc) ? req.query.desc : [req.query.desc];
